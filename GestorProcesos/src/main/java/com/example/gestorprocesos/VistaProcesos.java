...
public class VistaProcesos extends javax.swing.JFrame {
    ...
    private void btnDescargarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDescargarActionPerformed
        try {
            rest.findAll().getListaProcesos().forEach((proceso) -> {
                if (proceso != null) {
                    int rafaga = capturarRafaga(proceso.getDescripcion(), "o");
                    ...
                    datos[2] = rafaga;//Rafaga
                    datos[3] = rafaga;//Residuo rafaga
                    datos[4] = "Listo";
                    modelo.addRow(datos);
                    tablaProcesos.setModel(modelo);
                    Utilidad.quantum = proceso.getQuantum();
                    posicion++;
                }
            });
            if (cola.size() > -1) {
    ...
    private int capturarRafaga(String caracteres, String caracter) {
        int posicion = 0;
        int contador = 0;
        posicion = caracteres.toLowerCase().indexOf(caracter);
        while (posicion != -1) {
            contador++;
            posicion = caracteres.toLowerCase().indexOf(caracter, posicion + 1);
        }
        if (contador > 0) {
            return contador;
        } else {
