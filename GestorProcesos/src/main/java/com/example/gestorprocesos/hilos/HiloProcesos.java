 ...
 */
package com.example.gestorprocesos.hilos;

import com.example.gestorprocesos.Utilidad;
...
public class HiloProcesos implements Runnable {
                ...
                if (ResiduoRafaga > 0) {//Si la rafaga es mayor que 0 entonces el proceso se ejecuta
                    if (ResiduoRafaga > Quantum && prioridad == 0) {//Si la prioridad es 0, se ejecuta hasta terminar
                        for (int i = 0; i < Rafaga; i++) {
                            ...
                            cambioCaracter();//Realiza el cambio del caracter
    ...
    private void cambioCaracter() {
        StringBuilder builder = new StringBuilder();
        boolean state = true;
        boolean siCambio = false;
        int contadorCaracters = 0;
        while (state && contadorCaracters < procesoActual.getCaracteres().length()) {
            char cartcr = procesoActual.getCaracteres().charAt(contadorCaracters);
            char replace = procesoActual.getCaracter().charAt(0);
            if (cartcr == replace && !siCambio) {
                builder.append("x");
                procesoActual.settRafaga(procesoActual.gettRafaga() + 1);
                siCambio = true;
            } else {
                builder.append(String.valueOf(cartcr));
            }
            contadorCaracters++;
        }
        ...
        procesoActual.setCaracteres(builder.toString());
        guardarEnArchivo(builder.toString());
    }
